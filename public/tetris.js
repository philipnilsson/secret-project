  ...
  this.set = function (st) {
    ...
    for (i = 0; i < self.arr.length; i++) {
      if (Bacon._.all(self.arr[i])) {
        var line = self.arr.splice(i, 1)[0];
        lines.push(i--);
        res.powerups = 
          line.map(function(x){return x.special}).filter(function(x){return x})
      }
    }
    ...
    for (var ___ in lines) {
      ...
      for (var i = 0; i < self.h && r != 0; i++) 
      for (var j = 0; j < self.w && r != 0; j++) 
        if (self.arr[i][j]) { 
          r--;
          if (r == 0) {
